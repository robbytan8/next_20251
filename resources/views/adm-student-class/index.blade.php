          ...
          <div class="row align-items-center">
            <div class="col-md-12">
              <div class="page-header-title">
                <h5 class="m-b-10">Student Class Data</h5>
              </div>
              <ul class="breadcrumb">
                <li class="breadcrumb-item"><a href="#">Home</a></li>
                <li class="breadcrumb-item" aria-current="page">Student Class Management</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div class="col-sm-12">
          <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
              <h3>Student Class Data</h3>
              <a href="{{ route('adm-sclass.create') }}" class="btn btn-primary">Add Student Data</a>
            </div>
            <div class="card-body">
              <form method="GET" action="{{ route('adm-sclass.filter') }}" class="mb-3">
                @csrf
                <div class="row">
                  <div class="col-md-4">
                    <label for="periodId" class="form-label">Period Filter</label>
                    <select class="form-select" name="periodId" id="periodId">
                      <option value="" disabled selected>-- Select Period --</option>
                      <option value="0">All Period</option>
                      @foreach($periods as $period)
                        <option
                            value="{{ $period->id }}" {{ request('periodId') == $period->id ? 'selected' : '' }}>{{ $period->name }}</option>
                      @endforeach
                    </select>
                  </div>
              ...
              <div class="table-responsive">
                <table class="table table-striped table-hover" id="table-student">
                  ...
                  @foreach ($studentClasses as $studentClass)
                    <tr>
                      <td>{{ $studentClass->period->name }}</td>
                      <td>{{ $studentClass->student_id }}</td>
                      <td>{{ $studentClass->student->name }}</td>
                      <td>{{ $studentClass->class }}</td>
                      <td>
                        <form action="{{ route('adm-sclass.destroy', ['studentId' => $studentClass->student_id, 'periodId' => $studentClass->period_id, 'class' => $studentClass->class]) }}" method="POST" style="display:inline;">
                          @csrf
                          @method('DELETE')
